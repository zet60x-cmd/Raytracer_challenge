...
__host__ __device__ vector::vector(float x, float y, float z)
{
	this->x = x;
	this->y = y;
	this->z = z;
...
__host__ __device__ float vector::operator[](int i) const
{
	return body[i];
}
...
__host__ __device__ bool is_vector(const point& p)
{
	return (p.w == 0.0f);
}
...
__host__ __device__ vector operator-(const point& p1, const point& p2)
{
	return vector(p1.x - p2.x,
				  p1.y - p2.y,
				  p1.z - p2.z);
...
__host__ __device__ void point::print_point() const
{
...
__host__ __device__ void vector::print_vector() const
{
	printf("%f, %f, %f\n", this->x, this->y, this->z);
}
